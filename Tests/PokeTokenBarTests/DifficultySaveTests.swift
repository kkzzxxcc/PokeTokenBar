    ...
    func testLegacyRangeMigrationPreservesFractionAndDoesNotRepeatOnRestart() throws {
        for old in [0.0001, 0.01, 20.0] {
            for egg in [false, true] {
                ...
                // Legacy integer credits can round down by one token at tiny difficulty.
                let base = egg ? PokemonBalance.eggHatchThreshold : PokemonBalance.phaseThreshold(
                    rarity: .common, totalForms: 3, stageIndex: 0, growthMultiplier: 2)
                let oldThreshold = Double(base) * old
                let earnedFraction = Double(Int(oldThreshold * 0.5)) / oldThreshold
                XCTAssertEqual(egg ? s.eggProgress : s.progress, earnedFraction, accuracy: 0.000001)
