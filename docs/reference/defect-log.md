...
## Private 2.5.4 integration

The private fork retains one-tenth growth and shop thresholds. Official repeat-growth,
profile, Ditto and save-transfer fixtures assumed the official economy, so importing
those tests unchanged exercised overflow/graduation instead of their intended branches.
Scale their input and expected token amounts together while keeping identity, level,
and carryover assertions. Rare-candy fixtures use their actual legendary line rarity.
Legacy difficulty migration preserves the fraction representable by integer credits,
