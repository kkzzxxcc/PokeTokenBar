...
/// 이상한 사탕 밸런스 상수.
enum RareCandy {
    /// 사용 시 현재 포켓몬에 주입하는 XP(토큰 환산). 저사용량 커스텀 밸런스에서는 여러 단계가
    /// 한 번에 진행될 수 있다. applyUsage 로 주입 → 이월/진화/졸업 자동.
    static let xp = 100_000_000
    /// 주간 한도 100% 도달 시 지급 개수(세션급은 1개).
    static let weeklyGrant = 5
    /// 상점 구매가(재화 = 사용한 토큰: usedSinceInstall − spentTokens). 저사용량 커스텀 밸런스로
    /// 공식 가격의 1/10을 적용한다.
...
enum ShinyCharm {
    /// 상점 구매가. 앞으로의 모든 부화에 적용되는 영구 럭 업그레이드라 프리미엄(희귀 1마리 졸업분=300M).
